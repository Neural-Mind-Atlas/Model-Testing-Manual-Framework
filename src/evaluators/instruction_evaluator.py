...
class InstructionEvaluator:
            ...
            if metric_config.get("evaluation_method") == "model_based":
                ...
                try:
                    eval_response = self.evaluation_model.generate(
                        prompt=eval_prompt,
                        config={"system_prompt": "You are an expert evaluator assessing AI model responses for instruction following."}
                    )
    ...
    def _create_evaluation_prompt(self, prompt: str, response: str,
        ...
        where {min(scale)} means poor instruction following and {max(scale)} means perfect instruction following,
