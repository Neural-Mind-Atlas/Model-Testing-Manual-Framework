...
class ReasoningEvaluator:
            ...
            if metric_config.get("evaluation_method") == "model_based":
                ...
                try:
                    eval_response = self.evaluation_model.generate(
                        prompt=eval_prompt,
                        config={"system_prompt": "You are an expert evaluator assessing AI model reasoning quality."}
                    )
    ...
    def _create_evaluation_prompt(self, prompt: str, response: str,
        ...
        where {min(scale)} means poor reasoning and {max(scale)} means excellent reasoning,
